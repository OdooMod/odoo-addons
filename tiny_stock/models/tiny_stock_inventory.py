...
class TinyInventory(models.Model):
    _name = "tiny.stock.inventory"
    ...
    line_ids = fields.One2many(
        'tiny.stock.inventory.line', 'tiny_inventory_id', string='Inventories',
        copy=True, readonly=False,
        states={'done': [('readonly', True)]})
    move_ids = fields.One2many(
        'tiny.stock.move', 'tiny_inventory_id', string='Created Moves',
        states={'done': [('readonly', True)]})
    ...
    @api.one
    @api.constrains('filter', 'product_id', 'partner_id')
    def _check_filter_product(self):
        if self.filter == 'none' and self.product_id and self.location_id:
            return
    ...
    def action_inventory_line_tree(self):
        action = self.env.ref('tiny_stock.action_inventory_line_tree').read()[0]
        action['context'] = {
            'default_location_id': self.location_id.id,
            'default_product_id': self.product_id.id,
            'default_partner_id': self.partner_id.id,
            'default_tiny_inventory_id': self.id,
        }
        return action
    ...
    def _get_inventory_lines_values(self):
        ...
        self.env.cr.execute("""SELECT product_id, sum(product_qty) as product_qty, location_id, partner_id
            FROM tiny_stock_move
            WHERE %s
            GROUP BY product_id, location_id, partner_id """ % domain, args)
    ...
    def _get_exhausted_inventory_line(self, products, quant_products):
        ...
        for product in exhausted_products:
            vals.append({
                'tiny_inventory_id': self.id,
                'product_id': product.id,
                'location_id': self.location_id.id,
            })
        return vals

class TinyInventoryLine(models.Model):
    _name = "tiny.stock.inventory.line"
    _description = "Inventory Line"
    _order = "product_name ,tiny_inventory_id, location_name, product_code"

    tiny_inventory_id = fields.Many2one(
        'tiny.stock.inventory', 'Inventory',
    ...
    company_id = fields.Many2one(
        'res.company', 'Company', related='tiny_inventory_id.company_id',
        index=True, readonly=True, store=True)
    # TDE FIXME: necessary ? -> replace by location_id
    state = fields.Selection(
        'Status',  related='tiny_inventory_id.state', readonly=True)
    ...
    inventory_location_id = fields.Many2one(
        'stock.location', 'Location', related='tiny_inventory_id.location_id', related_sudo=False)
    ...
    @api.onchange('product_id', 'location_id', 'product_uom_id', 'partner_id')
    ...
    def create(self, values):
        ...
        existings = self.search([
            ...
            ('tiny_inventory_id.state', '=', 'confirm'),
            ('location_id', '=', values.get('location_id')),
            ('partner_id', '=', values.get('partner_id'))])
        res = super(TinyInventoryLine, self).create(values)
        if existings:
    ...
    def _get_move_values(self, qty, location_id, location_dest_id):
        self.ensure_one()
        return {
            'name': _('INV:') + (self.tiny_inventory_id.name or ''),
            'product_id': self.product_id.id,
            'product_uom': self.product_uom_id.id,
            'product_uom_qty': qty,
            'date': self.tiny_inventory_id.date,
            'company_id': self.tiny_inventory_id.company_id.id,
            'tiny_inventory_id': self.tiny_inventory_id.id,
            'state': 'done',
    ...
    def _generate_moves(self):
        moves = self.env['tiny.stock.move']
        for line in self:
